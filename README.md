...
入口の**展示室**に並ぶ厳選シーンをクリックすると**全画面没入**で立ち上がり、ドラッグで星を置いて軌道を観察できる。
...
操作: ドラッグで星追加（方向に初速）／ホイールでズーム／右・Shift ドラッグでパン／Space 一時停止／R リセット／Esc 展示室。
...
node test/bindings.test.mjs                          # バインディング整合（27 件）
```
